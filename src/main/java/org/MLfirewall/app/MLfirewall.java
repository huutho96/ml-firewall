...
/**
 * Application that calls a Machine Learning server to determine if traffic
...
public class MLfirewall {
	...
	private static final String MSG_BLOCKING = "Blocking Traffic";
	private static final String MSG_ALLOWING = "Allowing Traffic";
	private static final String MSG_FLOW_REMOVED = "Flow Removed";
	...
	// // // private final HashMultimap<DeviceId, PingRecord> pings =
	// HashMultimap.create();

	@Activate
	public void activate() {
		appId = coreService.registerApplication("org.MLfirewall.app", () -> log.info("MLfirewall registered"));
	...
	private void processPacket(PacketContext context) {
		...
		try {
			...
			String sendToServer = "srcMAC=" + srcMAC + ";dstMAC=" + dstMAC + ";srcIPv4=" + srcIPv4 + ";dstIPv4="
					+ dstIPv4 + ";srcPort=" + srcPort + ";dstPort=" + dstPort;
		...
		if (SERVER_DECISION_ALLOW) {
			...
			banPackets(deviceId, srcMAC, srcIPv4, srcPort, dstMAC, dstIPv4, dstPort);
			context.block();
		}
	...
	private void banPackets(DeviceId deviceId, MacAddress srcMAC, IpAddress srcIPv4, int srcPort, MacAddress dstMAC,
			IpAddress dstIPv4, int dstPort) {
		...
		if (dstPort == 0) {
			// Ignore port.
			selector = DefaultTrafficSelector.builder().matchEthSrc(srcMAC).matchEthDst(dstMAC).build();
		} else {
			// Include dstPort.
			selector = DefaultTrafficSelector.builder().matchEthSrc(srcMAC).matchEthDst(dstMAC).build();
		}
		...
		flowObjectiveService.forward(deviceId,
				DefaultForwardingObjective.builder().fromApp(appId).withSelector(selector).withTreatment(drop)
						.withFlag(ForwardingObjective.Flag.VERSATILE).withPriority(DROPALLOW_PRIORITY).add());
	}
	...
	private void allowPackets(DeviceId deviceId, MacAddress srcMAC, MacAddress dstMAC) {
		TrafficSelector selector = DefaultTrafficSelector.builder().matchEthSrc(srcMAC).matchEthDst(dstMAC).build();
		TrafficTreatment allow = DefaultTrafficTreatment.builder().build();

		flowObjectiveService.forward(deviceId,
				DefaultForwardingObjective.builder().fromApp(appId).withSelector(selector).withTreatment(allow)
						.withFlag(ForwardingObjective.Flag.VERSATILE).withPriority(DROPALLOW_PRIORITY).add());
	}
	...
	// // // return Objects.equals(this.src, other.src) && Objects.equals(this.dst,
	// other.dst);
	// // // }
	// // // }
